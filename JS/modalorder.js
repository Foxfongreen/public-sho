...
const deliveryCourierWrapper = document.querySelector(".delivery-wrapper-courier")
...
else if(event.currentTarget.elements.name.value !== "" && 
    event.currentTarget.elements.surname.value !== "" &&
    event.currentTarget.elements.tel.value !== "" &&
    event.currentTarget.elements.email.value !== "" &&
    event.currentTarget.elements.address.value !== "" &&
...
return acc += `${idx+1}. ${item.name} - ${item.quantity}шт. Сума: ${item.quantity*item.price}\n`
}, "")
const radioCheckedValue = form.querySelector("input[type='radio']:checked").value
const deliveryMessage = radioCheckedValue === "dept" ? "Доставка у відділення": "Курʼєрська доставка"
const deliveryText = radioCheckedValue === "dept" ? `Населений пункт: ${form.elements.city.value}\n№ Відділення: ${form.elements.department.value}\n`:
...
const data = JSON.stringify({chat_id: CHAT_ID_ORDER, parse_model:"html", text: message})

fetch(URL_ORDER, {method: "POST", headers: {"Content-Type":"application/json", "Content-Length":data.length}, body:data}).catch((error)=>{console.log(error.message);})
...
return `<li class="order-list-item" id=${item.id}>
    ...
    <div class="order-info-wrapper">
        ...
        <p class="order-item-total-price">${item.price*item.quantity}<span>₴</span></p>
        </div>
    </div>
...
if(event.target.classList.contains("order-counter-decrement")){
    ...
    const totalPrice = cartArr.reduce((acc, item)=>{
        ...
        orderTotalPrice.textContent = `${totalPrice},00₴`
        orderList.querySelector(`[id="${id}"]`).addEventListener("click", onRemoveBtnClick)
        orderList.querySelector(`[id="${id}"]`).addEventListener("click", onIncrementBtnClick)
        orderList.querySelector(`[id="${id}"]`).addEventListener("click", onDecrementBtnClick)
        if(cartArr.length){
            cartInnerWrapper.style.display = "flex"
        }else{
            cartInnerWrapper.style.display = "none"
